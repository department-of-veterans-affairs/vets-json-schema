...
const get = (object, path) => { // simpler than lodash's *and* empty path returns object
...
const is_definitions_path = path => (
  Array.isArray(path) &&
  path.length === 1 &&
  path[0] === 'definitions'
);
...
const inside_an_allOf_anyOf_or_oneOf = path => (
  last_element_of_path_is_an_integer(path) &&
    penultimate_element_is_either_allOf_anyOf_or_oneOf(path)
);
